...
use detector::CodePanel;

/// Scan all tmux panes and identify code panels
#[pyfunction]
fn scan_code_panels(py: Python) -> PyResult<Vec<PyObject>> {
    let runtime = tokio::runtime::Runtime::new().unwrap();
    let panels = runtime.block_on(detector::scan_code_panels());
...
fn panel_to_pydict(py: Python, panel: CodePanel) -> PyResult<&PyDict> {
    let dict = PyDict::new(py);
    ...
    dict.set_item("code_type", panel.code_type)?;
...
fn tmux_kanban_core(_py: Python, m: &PyModule) -> PyResult<()> {
    m.add_function(wrap_pyfunction!(scan_code_panels, m)?)?;
